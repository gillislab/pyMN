...
def topHits(adata, study_col = None, ct_col=None, cell_nv=None, mn_key='MetaNeighborUS', threshold=.95):
    ...
    if cell_nv is None:
    	assert mn_key in adata.uns_keys(), 'MetaNeighborUS resutls not stored in adata or passed as cell_nv'
    	cell_nv = adata.uns[mn_key].copy()
    	study_col = adata.uns[f'mn_key_params']['study_col']
    	ct_col = adata.uns[f'mn_key_params']['ct_col']

    assert np.all(cell_nv.index == cell_nv.column), 'cell_nv is does not have the same order in both the rows and columns'
