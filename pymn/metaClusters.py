...
def extractMetaClusters(best_hits, threshold = 0):
	comp = [np.array(list(l)) for l in nx.connected_components(make_graph(best_hits, threshold))]
	result = {}
	outliers = []
	n_mc = 1
...
def score_meta_clusters(meta_clusters, best_hits, adata, study_col, ct_col, outlier_label ='outliers'):
	...
	for module in modules.index:
		mc_summary[module] = {
			...
			'n_studies' : pheno.loc[modues[module], study_col].unique().shape[0],
			'score' : np.mean(best_hits.loc[modules[module], modules[module]].values)
		}
